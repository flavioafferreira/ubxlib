...
`ubxlib` Executa em um microcontrolador host e tem um periférico anexado. Essa configuração é muito comum em aplicativos incorporados. Um exemplo de uma configuração periférica do hospedeiro com EVK-NINA-B301 (Bluetooth 5.0) e EVK-R4 (SARA-R4 com 2G/3G/4G) no qual o `Ubxlib` Host configura uma conexão TCP é mostrada no Figura a seguir. Muitas outras combinações podem ser alcançadas, com os hosts e periféricos suportados nas tabelas na próxima seção.
...
Examples demonstrating the use of the APIs can be found in the [example](/example) directory.  Se você está usando o Zephyr ou o U-Blox [XPLR-IOT-1 platform](https://www.u-blox.com/en/product/xplr-iot-1) Você encontrará exemplos muito simples de instalar e usar in https://github.com/u-blox/ubxlib_examples_xplr_iot.
